...
def main_initialise():
    ...
    global screen, icon, mainClock, \
        font, username_main, password_main, \
        newpassword, loggedInUser, click
...
def user_account_successfully_deleted():
    ...
    while running:
        screen.fill((157, 193, 131))
        draw_text('Your account has been successfully', font, (0, 0, 0), screen, 10, 230)
        draw_text('deleted', font, (0, 0, 0), screen, 187, 265)
...
def main():
    if __name__ == "__main__":
        print("\n-~+=<({[ Program made by: Divij Kapoor ]})>=+~-")

        main_initialise()
    else:
        print("\n-~+=<({[ THIS FILE IS BEING IMPORTED ]})>=+~-\n")
        print("-~+=<({[ Program made by: Divij Kapoor ]})>=+~-")
        print("Initializing . . .")
